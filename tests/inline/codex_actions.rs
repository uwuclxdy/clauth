...
fn capture_creates_an_active_codex_profile() {
    ...
    codex_capture_into_profile(&mut cfg, &crate::profile::ProfileName::from("cdx-a"))
        .expect("capture");

    let profile = cfg
        .find(&crate::profile::ProfileName::from("cdx-a"))
...
fn capture_refuses_without_a_usable_live_login() {
    ...
    let mut cfg = empty_config();
    let err = codex_capture_into_profile(&mut cfg, &crate::profile::ProfileName::from("cdx-a"))
        .unwrap_err();
...
fn capture_dedups_the_account_across_codex_profiles() {
    ...
    let err = codex_capture_into_profile(&mut cfg, &crate::profile::ProfileName::from("cdx-b"))
        .unwrap_err();
...
fn capture_refuses_a_claude_profile_name() {
    ...
    crate::codex::write_live(&auth_bytes("at-alpha", "acct-alpha")).unwrap();
    let mut cfg = empty_config();
    cfg.add(crate::testutil::blank_profile(
        &crate::profile::ProfileName::from("work"),
    ));
    let err = codex_capture_into_profile(&mut cfg, &crate::profile::ProfileName::from("work"))
        .unwrap_err();
...
fn two_account_setup(cfg: &mut AppConfig) -> (Vec<u8>, Vec<u8>) {
    ...
    crate::codex::write_live(&alpha).unwrap();
    codex_capture_into_profile(cfg, &crate::profile::ProfileName::from("cdx-a"))
        .expect("capture alpha");
    crate::codex::write_live(&beta).unwrap();
    codex_capture_into_profile(cfg, &crate::profile::ProfileName::from("cdx-b"))
        .expect("capture beta");
    (alpha, beta)
}
...
fn switch_installs_the_target_chain() {
    ...
    let report = codex_switch_profile(
        &mut cfg,
        &crate::profile::ProfileName::from("cdx-a"),
        ForeignLivePolicy::Refuse,
    )
    .expect("switch");
    ...
    assert_eq!(
        crate::codex::read_profile_auth(&crate::profile::ProfileName::from("cdx-b"))
            .unwrap()
            .as_deref(),
        Some(&beta[..]),
...
fn cross_harness_switches_are_refused() {
    ...
    let mut cfg = empty_config();
    crate::codex::write_live(&auth_bytes("at-alpha", "acct-alpha")).unwrap();
    codex_capture_into_profile(&mut cfg, &crate::profile::ProfileName::from("cdx-a")).unwrap();
    cfg.add(crate::testutil::blank_profile(
        &crate::profile::ProfileName::from("work"),
    ));

    let err = switch_profile(&mut cfg, &crate::profile::ProfileName::from("cdx-a")).unwrap_err();
...
fn switch_refuses_a_quarantined_or_empty_target() {
    ...
    cfg.set_auth_broken(&crate::profile::ProfileName::from("cdx-a"), true);
    let err = codex_switch_profile(
        &mut cfg,
        &crate::profile::ProfileName::from("cdx-a"),
        ForeignLivePolicy::Refuse,
    ...
    cfg.set_auth_broken(&crate::profile::ProfileName::from("cdx-a"), false);

    // A codex profile with no stored login yet is not a switch target.
    let mut ghost = crate::testutil::blank_profile(&crate::profile::ProfileName::from("cdx-ghost"));
    ghost.harness = Harness::Codex;
    cfg.add(ghost);
    let err = codex_switch_profile(
        &mut cfg,
        &crate::profile::ProfileName::from("cdx-ghost"),
        ForeignLivePolicy::Refuse,
    )
    .unwrap_err();
...
fn clear_codex_auth_drops_store_and_marker_but_never_live() {
    ...
    codex_clear_profile_auth(&mut cfg, &crate::profile::ProfileName::from("cdx-b"))
        .expect("logout");
    assert!(
        crate::codex::read_profile_auth(&crate::profile::ProfileName::from("cdx-b"))
            .unwrap()
    ...
    assert!(
        cfg.find(&crate::profile::ProfileName::from("cdx-b"))
            .is_some(),
        "the profile shell survives"
    );
    assert_eq!(
    ...
    let err =
        codex_clear_profile_auth(&mut cfg, &crate::profile::ProfileName::from("work-missing"))
            .unwrap_err();
...
fn overwrite_captured_profile_refuses_a_codex_target() {
    ...
    let err = overwrite_captured_profile(
        &mut cfg,
        &crate::profile::ProfileName::from("cdx-a"),
        snapshot,
    )
    ...
    assert!(
        cfg.find(&crate::profile::ProfileName::from("cdx-a"))
            .unwrap()
            .api_key
            .is_none(),
...
fn hold_codex_lease(name: &str) -> std::fs::File {
    let dir = crate::runtime::codex_sessions_dir(&crate::profile::ProfileName::from(name)).unwrap();
    std::fs::create_dir_all(&dir).unwrap();
    let file =
...
fn standby_candidates_exclude_every_non_exclusive_chain() {
    ...
    cfg.set_auth_broken(&crate::profile::ProfileName::from("broken"), true);
    // tokenless: stored file with no refresh token — nothing to spend.
    crate::codex::write_profile_auth(
        &crate::profile::ProfileName::from("tokenless"),
...
fn switch_and_capture_refuse_while_a_refresh_holds_the_rotation_lock() {
    ...
    let _guard =
        crate::runtime::RotationGuard::acquire(&crate::profile::ProfileName::from("target"))
            .unwrap();
    ...
    let err = codex_capture_into_profile(&mut cfg, &crate::profile::ProfileName::from("target"))
        .expect_err("capture must refuse");
    assert!(err.to_string().contains("in flight"), "{err}");
}
...
fn switch_and_capture_refuse_a_leased_profile() {
    ...
    let err = codex_switch_profile(
        &mut cfg,
        &crate::profile::ProfileName::from("leased"),
        ForeignLivePolicy::Archive,
    )
    .expect_err("switch must refuse a leased target");
    assert!(err.to_string().contains("clauth start"), "{err}");

    let err = codex_capture_into_profile(&mut cfg, &crate::profile::ProfileName::from("leased"))
        .expect_err("capture must refuse");
    assert!(err.to_string().contains("clauth start"), "{err}");
}
...
fn browser_login_store_never_touches_live_or_the_active_slot() {
    ...
    codex_store_browser_login(
        &mut cfg,
        &crate::profile::ProfileName::from("cdx-new"),
        &minted,
    )
    .expect("store");

    assert_eq!(
        crate::codex::read_profile_auth(&crate::profile::ProfileName::from("cdx-new"))
            .unwrap()
            .as_deref(),
        Some(&minted[..])
    );
    ...
    assert_eq!(
        cfg.find(&crate::profile::ProfileName::from("cdx-new"))
            .map(|p| p.harness),
        Some(Harness::Codex)
    );
}
...
fn browser_login_store_dedups_accounts_and_heals_quarantine() {
    ...
    let err =
        codex_store_browser_login(&mut cfg, &crate::profile::ProfileName::from("second"), &dup)
            .expect_err("dup refused");
    assert!(err.to_string().contains("existing"), "{err}");

    // Re-auth of a quarantined profile heals the flag.
    cfg.set_auth_broken(&crate::profile::ProfileName::from("existing"), true);
    let fresh = auth_bytes("at-fresh2", "acct-existing");
    codex_store_browser_login(
        &mut cfg,
        &crate::profile::ProfileName::from("existing"),
        &fresh,
    )
    .expect("re-auth in place");
    assert!(
        !cfg.is_auth_broken(&crate::profile::ProfileName::from("existing")),
...
fn browser_login_store_rejects_a_claude_name_and_shell_bytes() {
    ...
    let mut cfg = empty_config();
    cfg.add(crate::testutil::blank_profile(
        &crate::profile::ProfileName::from("claude-p"),
    ));

    let minted = auth_bytes("at-m", "acct-m");
    let err = codex_store_browser_login(
        &mut cfg,
        &crate::profile::ProfileName::from("claude-p"),
        &minted,
    )
    .expect_err("cross-harness");
    assert!(err.to_string().contains("claude profile"), "{err}");
