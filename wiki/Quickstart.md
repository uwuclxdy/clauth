...
- **`daemon --listen`** also serves the REST API over TLS: the status feed and the account switch. It is off unless asked for, and every route needs the bearer token, so anything holding that token can act on that daemon. `--print-token` and `--rotate-token` manage it and work whether or not a daemon is running — a rotation takes effect against a running one, without a restart. TLS comes from this host's lego certificate, or from `--cert`/`--key` when the host's own name resolves to no certificate. Full detail in [Daemon](Daemon).
