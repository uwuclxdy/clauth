...
| `~/.clauth/profiles/<name>/mcp-logins.json` | those MCP-server logins alone (or the keys `carried_credential_keys` lists), parked whenever the account stores no Claude login of its own and merged back once it regains one |
| `~/.clauth/profiles/<name>/session-token.json` | a long-lived `claude setup-token` login, when captured, plus the keys `carried_credential_keys` lists |
...
`carried_credential_keys` in `profiles.toml` ([Configuration](Configuration#profilestoml)) widens this on purpose: every key it lists is copied the same way, into every account's `credentials.json` and `session-token.json`, and into `mcp-logins.json` when a profile stores no login. A listed `designOauth` is a Claude Design login with its own refresh token, so that token then sits under every account too, and Claude Code rotates it on refresh: a copy that stays behind in a store you do not switch into goes stale. On macOS the same list governs what crosses inside the Keychain item, and every sign-out of the item (a switch onto an account that stores no login, a wrap-off, deleting the active profile, a logout) keeps the listed keys in it, as it keeps the MCP logins. That differs from Claude Code's own logout, which deletes a design login; on macOS the item is that login's only copy. Your Claude logins themselves (`claudeAiOauth`) are never copied. Unsetting the list stops the copying but removes no copy already made. Inside `clauth start` sessions the list is not honored yet (on macOS their per-session Keychain items keep the built-in carry), and a session running on a setup token that refreshes a carried login loses the refreshed copy, so leave the list unset on a host running such sessions.
