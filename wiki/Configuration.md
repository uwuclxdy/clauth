...
| `carried_credential_keys` | list | unset | replaces the built-in carry (`mcpOAuth` alone) with the keys of Claude Code's credential store a switch copies from the live login onto the incoming account, e.g. `["mcpOAuth", "designOauth"]` to keep a `/design-login` across accounts. Once set, even to `["mcpOAuth"]`, it also reaches a setup-token sidecar and survives its re-mint. `claudeAiOauth` is never carried; `organizationUuid`, `trustedDeviceToken` and `enterpriseGateway` belong to one account and should stay off it. On macOS it applies to the Keychain item too, and every sign-out of the item (a switch onto an account that stores no login, a wrap-off, deleting the active profile, a logout) keeps the listed keys in it, like the MCP logins. The list replaces the built-in one, so keep `mcpOAuth` in it to go on carrying MCP logins. Not honored inside `clauth start` sessions (on macOS their per-session Keychain items keep the built-in carry); do not set it where `clauth start` sessions run on a setup token, since a session there can spend a carried refresh token ([Security](Security#mcp-server-logins)) |
