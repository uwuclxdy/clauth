...
| `chatgpt.com/backend-api/wham/rate-limit-reset-credits` and its `/consume` | only when you run `clauth use-reset <name>`: lists that account's banked usage-limit resets and, after you confirm, spends one, with the profile's stored access token. Nothing polls or retries it. |
